...
왼쪽 위의 위치에서 오른쪽 아래의 위치로 이동하는 데, 밟은 숫자의 합이 최소가 되는 경로를 찾는 문제

다익스트라 알고리즘을 이용하여 풀이하였다.

2차원 배열을 이용하여 모두 최대값으로 초기화를 시키고, 좌측 상단부터 0을 넣고, 다익스트라 알고리즘을 이용하여, 최단 경로를 찾아나간다.

가장 처음 풀때는, 2차원 배열에서 최솟값을 찾기위하여 이중for문을 이용하여 풀이하였지만, PriorityQueue를 이용한다면 훨씬 빠른 속도로 풀이할 수 있었다.
...
N개의 섬의 위치와 섬의 거리에 따른 세율이 주어진다. 모든 섬을 잊는 다리를 최소 비용으로 짓는다고 할 때, 최소 비용을 구하는 문제

각 섬의 좌표를 저장하고, 각 섬을 정점으로 모든 정점을 잇는 간선이 있다고 생각하고 인접 행렬을 채웠다. 이때, 간선의 가중치는 두 섬의 거리의 제곱에 입력받은 세율을 곱한 값이다.

위 인접행렬을 바탕으로 프림 알고리즘을 사용하여 최소 비용을 구하였다.
...
입국 심사 창구가 N개 주어지고 각 창구에서 한 사람을 심사하는 시간이 주어진다. 각 사람은 자신이 가장 빠르게 심사를 끝낼 수 있는 곳을 간다고 할 때, M명을 처리하는 데 걸리는 시간을 출력하는 문제

처음에는 각 인원이 들어갈 수 있는 곳에 하나씩 넣어주면서, 시간을 늘려가며 M명의 사람을 처리하면 끝나는 반복문을 이용하여 풀이하려 했으나, 시간 초과가 발생하였다.

따라서 시간이 주어졌을 때, 모든 창구가 해당 시간 동안 해결하는 사람들의 수를 더하여, M보다 크면, 시간을 줄이고, M보다 작으면 시간을 늘리는 방식으로 풀이하였다.

시간을 1씩 증가, 감소시키면 시간초과가 발생할 것이 뻔하기 때문에 이분 탐색을 통해서, M명을 처리할 수 있는 가장 작은 시간을 찾아갔다.
