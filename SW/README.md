...
방범 서비스를 제공하는 회사에서 마을에 방범 서비스를 제공하면서, 그 범위에 따른 비용이 발생하고, 해당 범위 안에 있는 집들로 부터 M의 비용을 받는 다고 할 때, 회사가 손해를 보지 않으면서 가장 많은 집에 방범 서비스를 제공할 때, 집의 수를 구하는 문제

완전 탐색으로 풀이하였다.

모든 칸에 대해서 1부터 모든 칸을 덮을 수 있는 범위까지의 방범 서비스를 제공해 보면서, 손해를 보지 않으며 가장 많은 집에 서비스를 제공하는 지 계산하는 문제

방범 서비스를 제공하는 비용은 범위^2 + (범위-1)^2 임을 빠르게 파악한 것이 주요했다.
